...
* If updates are made to the CSV, any of the CRDs or RBAC (any changes in
  ...
  are up-to-date (note that the version here is not important as it will be
  replaced before publishing the bundle downstream.  Using 0.0.0 to make it
  obvious that the version in source will not be the same one that will be
  published):
...
* Generate a bundle at a specific version (0.4.1 in this example) that will be
  published to channel `stable` and channel `release-0.4`, while making the
  default channel `stable`:

  `$ make bundle VERSION=0.0.1 CHANNELS=stable,release-0.4
